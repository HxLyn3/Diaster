...
class DiagGaussian(nn.Module):
    ...
    def __init__(
        ...
        if conditioned_sigma:
            ...
            self.sigma_param = nn.Parameter(torch.zeros(output_dim, 1), requires_grad=False)
        self._unbounded = unbounded
        self._max = nn.Parameter(torch.as_tensor(max_mu), requires_grad=False)
        self._sigma_min = nn.Parameter(torch.as_tensor(sigma_min), requires_grad=False)
        self._sigma_max = nn.Parameter(torch.as_tensor(sigma_max), requires_grad=False)
...
class ProbActor(nn.Module):
    ...
    def __init__(self, obs_shape, hidden_dims, action_dim, max_action=1.0, sigma_cond=False, sigma_min=-20, sigma_max=2):
...
class DeterActor(nn.Module):
    ...
    def __init__(self, obs_shape, hidden_dims, action_dim, max_action):
        ...
        self.max_action = nn.Parameter(torch.as_tensor(max_action), requires_grad=False)
